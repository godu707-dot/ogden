import { render, screen } from '@testing-library/react'
import { vi, describe, it, expect, beforeEach } from 'vitest'
import ExecutionHistory from '../../components/execution-history'
import { ethers } from 'ethers'

vi.mock('../../lib/contract', () => ({
  listenToContractEvents: vi.fn()
}))

describe('ExecutionHistory', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('renders empty state', () => {
    render(<ExecutionHistory />)
    expect(screen.getByText('No executions yet')).toBeInTheDocument()
  })

  it('displays arbitrage executions', () => {
    const mockEvent = {
      tokenBorrow: '0xTokenA',
      amount: ethers.utils.parseEther('1').toString(),
      profit: ethers.utils.parseEther('0.1').toString(),
      dexPath: ['UniswapV2', 'SushiSwap'],
      timestamp: Date.now()
    }

    render(<ExecutionHistory initialEvents={[mockEvent]} />)
    
    expect(screen.getByText(/Profit: /)).toHaveTextContent(mockEvent.profit)
    expect(screen.getByText(/Token: /)).toHaveTextContent(mockEvent.tokenBorrow)
    expect(screen.getByText(/Dex path: /)).toHaveTextContent('UniswapV2 → SushiSwap')
  })

  it('limits history to 50 items', () => {
    const events = Array(60).fill(null).map((_, i) => ({
      profit: i.toString(),
      timestamp: Date.now() + i
    }))

    render(<ExecutionHistory initialEvents={events} />)
    const items = screen.getAllByText(/Profit: /)
    expect(items).toHaveLength(50)
  })
})